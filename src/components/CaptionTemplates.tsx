...
export function CaptionTemplates({ userId, companyName, companySummary, onTemplateSelect }: CaptionTemplatesProps) {
    ...
    const generateTemplates = async () => {
        try {
            setLoading(true);
            const response = await fetch('http://localhost:5001/api/generate-templates', {
                method: 'POST',
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-white/60 text-sm font-medium">Quick Suggestions</h4>
                </div>
                <button
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {loading ? (
                    ...
                    Array.from({ length: 6 }).map((_, i) => (
                        <div
                            key={i}
                            className="flex-shrink-0 h-9 w-32 bg-white/5 rounded-full animate-pulse"
                        />
                    ))
                ) : (
